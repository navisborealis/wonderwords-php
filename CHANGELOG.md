...
### Changed
- **BREAKING (Environment)**: Bumped minimum PHP requirement from `7.1` to `7.4`.
- **CI**: Updated GitHub Actions to Node 24 compatible versions (`actions/checkout@v6`, `nick-fields/retry@v4`) and removed the daily cron trigger.

## [1.3.0] - 2026-08-08

### Added
- **FakerPHP Integration**: Added a custom Faker Provider (`NavisBorealis\WonderwordsPhp\Faker\WonderWordsProvider`) to seamlessly generate words, phrases, and sentences directly through `$faker`.
- **Documentation**: Added a Quick Start section with examples for usernames, entity naming, and URL slugs. Added badges for PHP version, license, test coverage, and static analysis.

### Changed
- **Documentation**: Corrected phrase method signature and removed outdated note about sentences being in development.

## [1.2.1] - 2026-08-01

### Fixed
- Removed accidentally committed release notes file from the repository to clean up the package source.
